...
void Button::setPosition(sf::Vector2f point) {
	...
	float div = 2 + btnHeight / btnWidth;

	float xPos = (point.x + btnWidth / div) - (text.getLocalBounds().width / 2) + 10;
	float yPos = (point.y + btnHeight / div) - (text.getLocalBounds().height / 2)-1;
